...
xfblasStatus_t xfblasCreate(const char* xclbin,
    ...
    } else if (engineName == XFBLAS_ENGINE_FCN) {
        if (ConfigDict::instance().m_dict["BLAS_runFcn"] != "1") {
            return XFBLAS_STATUS_INVALID_VALUE;
        }
...
xfblasStatus_t xfblasMallocRestricted(
    ...
    } else if (ConfigDict::instance().m_dict["BLAS_runFcn"] == "1") {
        unsigned long long l_bufSize = rows * lda * elemSize;
        xfblasStatus_t l_status =
            BLASHostHandle::instance().m_handlePtr[deviceIndex][kernelIndex]->allocMatRestricted(A, A, l_bufSize);
        return l_status;
...
xfblasStatus_t xfblasFcn(int m,
                         int n,
                         int k,
                         int alpha,
                         void* A,
...
xfblasStatus_t xfblasFcnByAddress(unsigned int l_aOff,
                                  unsigned int l_bOff,
                                  unsigned int l_cOff,
                                  unsigned int l_xOff,
                                  unsigned int p_m,
