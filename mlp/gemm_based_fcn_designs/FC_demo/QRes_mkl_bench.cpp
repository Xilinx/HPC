...
int main(int argc, char** argv) {
    ...
    for (int ll = 0; ll < LoopCount; ll++) {
        ...
        for (int kn = 0; kn <= NNnumber - 1; kn++) {
            ...
            if (DUMP_RESULT) {
                outFile.open(data_dir + "matb1_" + to_string(kn) + ".bin", ofstream::binary);
                ...
                outFile.write((char*)TwoLayerCase[kn].IW, sizeof(XFBLAS_dataType) * NNinputs * L1size);
                outFile.close();
            }

            GEMM_MKL(NNtraces, L1size, 1, alpha, beta, OnesTraces, TwoLayerCase[kn].b1,
                     L1temp); // Copy bias1 with number of traces

            GEMM_MKL(NNtraces, L1size, NNinputs, alpha, beta, InputTable, TwoLayerCase[kn].IW, L1temp);
            ...
            if (DUMP_RESULT) {
                ...
                outFile.open(data_dir + "matb2_" + to_string(kn) + ".bin", ofstream::binary);
            ...
            GEMM_MKL(NNtraces, L2size, 1, alpha, beta, OnesTraces, TwoLayerCase[kn].b2,
                     L2temp); // Copy bias2 with number of traces

            GEMM_MKL(NNtraces, L2size, L1size, alpha, beta, L1temp, TwoLayerCase[kn].LW21,
            ...
            if (DUMP_RESULT) {
                ...
                outFile.open(data_dir + "matb3_" + to_string(kn) + ".bin", ofstream::binary);
            ...
            GEMM_MKL(NNtraces, NNoutputs, 1, alpha, beta, OnesTraces, TwoLayerCase[kn].b3,
                     Otemp); // Copy bias3 with number of traces

            GEMM_MKL(NNtraces, NNoutputs, L2size, alpha, beta, L2temp, TwoLayerCase[kn].LW32,
