...
int main(int argc, char** argv) {
    ...
    XFBLAS_dataType *InputTable, *InputTable2, *OutputTable, *OnesTraces, alpha = 1., beta = 1.;
    ...
    if (inFile.is_open()) {
        ...
        for (int kn = 0; kn <= NNnumber - 1; kn++) {
            ...
            for (int k3 = 0; k3 < NNinputs; k3++)   // Columns
                ...
                {
                    TwoLayerCase[kn].IW[k2 + (int)L1size * k3] =
            ...
            for (int k3 = 0; k3 < L1size; k3++)     // Columns
                ...
                {
                    TwoLayerCase[kn].LW21[k2 + (int)L2size * k3] = memblock[kindex++]; // matrices saved by column
                }
            // Reading b2
            ...
            for (int k3 = 0; k3 < L2size; k3++) // Columns
                for (int k2 = 0; k2 < NNoutputs; k2++) {
                    TwoLayerCase[kn].LW32[k2 + (int)NNoutputs * k3] = memblock[kindex++]; // matrices saved by column
                }
            // Reading b3
            for (int k2 = 0; k2 < NNoutputs; k2++) {
    ...
    for (int ll = 0; ll < LoopCount; ll++) {
        ...
        if (inFilein.is_open()) {
            ...
            for (int k2 = 0, k4 = 0; k2 < NNtraces; k2++) // Rows
                ...
                {
                    InputTable[k2 * (int)NNinputsData + k3] = memblock[k4++];
                }

            if (DUMP_RESULT) {
                outFile.open(data_dir + "mat_input_" + to_string(NNtraces) + ".bin", ofstream::binary);
            ...
            OnesTraces = createMat(NNtraces, 1, false, 1.0);
        ...
        // Cold Start
        chrono::duration<double> l_durationSec_cold = chrono::seconds(0);
        for (int kn = 0; kn <= NNnumber - 1; kn++) {
            // Create temp layer results
            XFBLAS_dataType *L1temp, *L2temp, *Otemp;
            XFBLAS_dataType* l_IW = createMat(L1size, NNinputs);    // new double[NNinputs*L1size];
            XFBLAS_dataType* l_b1 = createMat(L1size, 1);           // new double[L1size];
            XFBLAS_dataType* l_LW21 = createMat(L2size, L1size);    // new double[L1size*L2size];
            XFBLAS_dataType* l_b2 = createMat(L2size, 1);           // new double[L2size];
            XFBLAS_dataType* l_LW32 = createMat(NNoutputs, L2size); // new double[L2size*NNoutputs];
            XFBLAS_dataType* l_b3 = createMat(NNoutputs, 1);        // new double[NNoutputs];
            XFBLAS_dataType* l_InputTable = createMat(NNtraces, NNinputsData);
            XFBLAS_dataType* l_OnesTraces = createMat(1, NNtraces, false, 1.0);
            L1temp = createMat(L1size, NNtraces);
            L2temp = createMat(L2size, NNtraces);
            Otemp = createMat(NNoutputs, NNtraces);

            memcpy(reinterpret_cast<char*>(l_IW), reinterpret_cast<char*>(TwoLayerCase[kn].IW), L1size*NNinputs*sizeof(XFBLAS_dataType));
            memcpy(reinterpret_cast<char*>(l_b1), reinterpret_cast<char*>(TwoLayerCase[kn].b1), L1size*sizeof(XFBLAS_dataType));
            memcpy(reinterpret_cast<char*>(l_LW21), reinterpret_cast<char*>(TwoLayerCase[kn].LW21), L2size*L1size*sizeof(XFBLAS_dataType));
            memcpy(reinterpret_cast<char*>(l_b2), reinterpret_cast<char*>(TwoLayerCase[kn].b2), L2size*sizeof(XFBLAS_dataType));
            memcpy(reinterpret_cast<char*>(l_LW32), reinterpret_cast<char*>(TwoLayerCase[kn].LW32), L2size*NNoutputs*sizeof(XFBLAS_dataType));
            memcpy(reinterpret_cast<char*>(l_b3), reinterpret_cast<char*>(TwoLayerCase[kn].b3), NNoutputs*sizeof(XFBLAS_dataType));
            memcpy(reinterpret_cast<char*>(l_InputTable), reinterpret_cast<char*>(InputTable), NNtraces*NNinputsData*sizeof(XFBLAS_dataType));
            ...
            if (DUMP_RESULT) {
                ...
                outFile.write((char*)TwoLayerCase[kn].IW, sizeof(XFBLAS_dataType) * L2size * L1size);
                outFile.close();
            }

            l_tp[0] = chrono::high_resolution_clock::now();
            GEMM_MKL(NNtraces, L1size, 1, alpha, beta, l_OnesTraces, l_b1,
                     L1temp); // Copy bias1 with number of traces

            GEMM_MKL(NNtraces, L1size, NNinputs, alpha, beta, l_InputTable, l_IW, L1temp);
            ...
            GEMM_MKL(NNtraces, L2size, 1, alpha, beta, l_OnesTraces, l_b2,
                     L2temp); // Copy bias2 with number of traces

            GEMM_MKL(NNtraces, L2size, L1size, alpha, beta, L1temp, l_LW21,
            ...
            GEMM_MKL(NNtraces, NNoutputs, 1, alpha, beta, l_OnesTraces, l_b3,
                     Otemp); // Copy bias3 with number of traces

            GEMM_MKL(NNtraces, NNoutputs, L2size, alpha, beta, L2temp, l_LW32,
            ...
            if (DUMP_RESULT) {
                ...
                for (int k2 = 0; k2 < NNtracesOut; k2++)   // Rows
                    for (int k3 = 0; k3 < NNoutputs; k3++) // Columns
                    {
                        tempDiff = OutputTable[k2 * (int)NNOutputsData + k3 + kn * NNoutputs] -
                                   Otemp[k2 * (int)NNoutputs + k3];
            ...
            chrono::duration<double> l_durationSec_NN = l_tp[1] - l_tp[0];
            l_durationSec_cold += l_durationSec_NN;
            //cout << "Time (secs) for NN " << kn << " : " << (double)(l_durationSec_NN.count()) << endl;
            free(L1temp);
            free(L2temp);
            free(Otemp);
            free(l_IW);
