...
int main(int argc, char** argv) {
    ...
    PCGImpl<CG_dataType, CG_parEntries, CG_instrBytes, SPARSE_accLatency, SPARSE_hbmChannels, SPARSE_maxRows,
            SPARSE_maxCols, SPARSE_hbmMemBits> l_pcg(l_deviceId, binaryFile);
    ...
    showTimeData("Matrix partition time: ", l_timer[1], l_timer[2], &l_mat_partition_time);
    l_pcg.sendMat();
    showTimeData("Send Mat time: ", l_timer[2], l_timer[3]);
    l_pcg.setVec(l_matInfo.m_m, l_b.data(), l_diagA.data());
    showTimeData("Vector initialization & transmission time: ", l_timer[3], l_timer[4]);
    Results<CG_dataType> l_res = l_pcg.run(l_maxIter, l_tol);
    double l_runTime = 1;
    showTimeData("PCG run time: ", l_timer[4], l_timer[5], &l_runTime);
    double l_h2d_time = 0;
    showTimeData("Host to device data transfer time: ", l_timer[2], l_timer[4], &l_h2d_time);
