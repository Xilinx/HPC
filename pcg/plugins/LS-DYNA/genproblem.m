#!/usr/bin/env octave
arg_list = argv();
npmin = str2num(arg_list{1});
npmax = str2num(arg_list{2});
nums   =  str2num(arg_list{3}); 
nps = linspace(npmin, npmax, nums);
mode =   0 ; % 0: implicit; 1: thermal
...
tic;
for id = 1:nums
    np = nps(id); % Number of points along one dimension
    new_dir = ['ThermalCube' num2str(np)];
    mkdir(new_dir) ;
    old_dir = cd(new_dir);
    nvtx = np^3 ;
    x = zeros(nvtx, 1) ;
