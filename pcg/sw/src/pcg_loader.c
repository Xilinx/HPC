...
XJPCG_Status_t xJPCG_createHandle(XJPCG_Handle_t** handle, const char* xclbinPath) {
    typedef XJPCG_Status_t (*CreateFunc)(XJPCG_Handle_t**, const char*);
    CreateFunc pCreateFunc = (CreateFunc)xilinx_apps_getCDynamicFunction("xJPCG_createHandle");
    if (!pCreateFunc) return XJPCG_STATUS_DYNAMIC_LOADING_ERROR;
    return pCreateFunc(handle, xclbinPath);
}
...
XJPCG_Status_t xJPCG_destroyHandle(XJPCG_Handle_t* handle) {
    ...
    if (!pDestroyFunc) return XJPCG_STATUS_DYNAMIC_LOADING_ERROR;
...
XJPCG_Status_t xJPCG_getMetrics(const XJPCG_Handle_t* handle, XJPCG_Metric_t* metric) {
    ...
    if (pGetMetrics) return pGetMetrics(handle, metric);
...
const char* xJPCG_getLastMessage(const XJPCG_Handle_t* handle) {
    ...
    if (pGetLastMessage) return pGetLastMessage(handle);
...
const char* xJPCG_getErrorString(XJPCG_Status_t code) {
    ...
    if (code == XJPCG_STATUS_DYNAMIC_LOADING_ERROR) return "Dynamic loading error";

    GetErrorString pGetErrorString = (GetErrorString)xilinx_apps_getCDynamicFunction("xJPCG_getErrorString");
    if (pGetErrorString) return pGetErrorString(code);
...
XJPCG_Status_t xJPCG_cscSymSolver(XJPCG_Handle_t* handle,
    ...
    typedef XJPCG_Status_t (*ApiFunc)(XJPCG_Handle_t*, int64_t, int64_t, const int64_t*, const int64_t*, const double*,
                                      const double*, const double*, const double*, const int64_t, const double,
                                      int64_t*, double*, const XJPCG_Mode_t);
    ApiFunc pApiFunc = (ApiFunc)xilinx_apps_getCDynamicFunction("xJPCG_cscSymSolver");
    if (!pApiFunc) return XJPCG_STATUS_DYNAMIC_LOADING_ERROR;
...
XJPCG_Status_t xJPCG_cooSolver(XJPCG_Handle_t* handle,
                               ...
                               double* p_res,
                               const XJPCG_Mode_t mode) {
    typedef XJPCG_Status_t (*ApiFunc)(const XJPCG_Handle_t*, uint32_t, uint32_t, const uint32_t*, const uint32_t*,
                                      const double*, const double*, const double*, const double*, const uint32_t,
                                      const double, uint32_t*, double*, const XJPCG_Mode_t);
