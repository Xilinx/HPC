...
*/
#ifndef HPC_MLP_GEN_FCN_H
#define HPC_MLP_GEN_FCN_H
...
/** 
 * @brief compute the golden reference of operation C = activation(A*B+X), current supported activation function includes: sigmoid, tansig and Relu
 *
 * @tparam T data type, currently support float32
 * @param p_A matrix A pointer
 * @param p_B matrix B pointer
 * @param P_X matrix X pointer
 * @param p_C matrix C pointer
 * @param p_PReluVAL, if bit_0 == 0, results go through activation function, otherwise return the results. Macro MLP_RELU, MLP_tansig are used to control different activation fucntions used
 */
template <typename T>
   ...
   public:
    ...
    void addInstrFromFile(ProgramType& p_Program, std::string p_FileName, bool p_WithGolden) {
        ...
        if (l_good) {
            ...
            std::cout << "INFO " << l_handleC << " " << l_cM << " " << l_cN << "\n";
            assert(l_cM == l_M);
            assert(l_cN == l_N);
            ...
            if (p_WithGolden) {
                fcn_ref<BLAS_dataType>(l_matA, l_matB, l_matC, l_matX, l_PRelu);
            }
            ...
            std::cout << "Added FCN" << l_M << "x" << l_K << "x" << l_N << std::endl; 
            l_fs.close();
        } else {
    ...
    void addInstrFromFiles(unsigned int l_instrIndex,
        ...
        if (l_good) {
            ...
            l_fs_ins >> l_unused >> l_PRelu;
            std::cout << "INFO " << l_unused << " " << l_PRelu << "\n";
            // read A dimensions
    ...
    void addInstr(ProgramType& p_Program,
        ...
        if (p_WithGolden) {
            fcn_ref<BLAS_dataType>(l_matA, l_matB, l_matC, l_matX, p_PReluVal);
        }
        std::cout << "Added FCN" << p_M << "x" << p_K << "x" << p_N << std::endl;
    }
    ...
    void show(ProgramType& p_Program, FcnArgsType p_FcnArgs) {
        ...
        int32_t l_unused = p_FcnArgs.m_postScale;
