...
class SparseMatrix {
   public:
    ...
    void set_row(void* p_rowIdx) {
        m_row_list.resize(m_nnz);
        memcpy(reinterpret_cast<char*>(&m_row_list[0]), reinterpret_cast<char*>(p_rowIdx), m_nnz * sizeof(int));
        m_minRowId = *(min_element(m_row_list.begin(), m_row_list.end()));
    }

    void set_col(void* p_colIdx) {
        m_col_list.resize(m_nnz);
        memcpy(reinterpret_cast<char*>(&m_col_list[0]), reinterpret_cast<char*>(p_colIdx), m_nnz * sizeof(int));
        m_minColId = *(min_element(m_col_list.begin(), m_col_list.end()));
    }

    void set_data(void* p_data) {
        m_data_list.resize(m_nnz);
        memcpy(reinterpret_cast<char*>(&m_data_list[0]), reinterpret_cast<char*>(p_data), m_nnz * sizeof(double));
    }
