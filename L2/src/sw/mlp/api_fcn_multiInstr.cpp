...
int main(int argc, char** argv) {
    ...
    if (argc < 6) {
        cerr << "Usage:\n"
                  << "  mlp_api_fcn_multiInstr.exe <path/mlp.xclbin> [M K N is_linear HandleA HandleB HandleC HandleX]\n"
    ...
    if ((argc - 2) % 10 == 0) {
        ...
        l_instrCount = ((argc - 2) / 8 > 1) ? ((argc - 2) / 8) : 1; // number of instructions

    } else {
        cerr << "  Each instruction has to contain [M K N has_activation HandleA HandleB HandleC HandleX] "
                  << "or [M K N matAFile matBFile matXFile HandleA HandleB "
                     "HandleC HandleX]\n";
        exit(2);
    }

    if (l_instrCount > BLAS_numInstr) {
        cerr << "  Number of instructions exceeds the capacity in the hardware instruction memory.\n";
        exit(2);
    }
    ...
    int32_t l_postScaleVal = 1;
    int32_t l_postScaleShift = 0;
    int32_t l_postScale[l_instrCount];
    int16_t l_PReluScale=1;
    ...
    cout << "FCN C++ API example using accelerator image " << l_xclbinFile << endl;
    ...
    for (int i = 0; i < BLAS_numKernels; i++) {
        ...
        for (unsigned int index = 0; index < l_instrCount; index++) {
            if (readFromFiles) {
                ...
                l_k[index] = atoi(argv[l_argIdx++]);
                l_n[index] = atoi(argv[l_argIdx++]);
                l_lda[index] = l_k[index];
                l_ldb[index] = l_n[index];
                l_ldc[index] = l_n[index];
                l_ldx[index] = l_n[index];
                l_postScaleVal = 1;
                l_postScaleShift = 0;
                l_postScale[index] = (l_postScaleVal << 8) | (l_postScaleShift & 0x000000ff);
                l_PReluScale = 1;
                string l_isLinear = argv[l_argIdx++];
                transform(l_isLinear.begin(), l_isLinear.end(), l_isLinear.begin(), ::toupper);
                if (l_isLinear == "TRUE") {
                    l_PReluAlpha = 1;
                }
                else {
    ...
    // golden program
    l_argIdx = 2;
    cout << "Calculate golden result on host, for large matrix size, this will take long time.\n" << endl;
    ...
    string kernelNames[BLAS_numKernels];
    ...
    for (int i = 0; i < BLAS_numKernels; ++i) {
        for (unsigned int j = 0; j < l_instrCount; ++j) { // number of instructions
            ...
            cout << string("cycles in kernel ") << i << " " << l_cycleCount << endl;
    ...
    if (readFromFiles) {
        cout << string("DATA_CSV:,DdrWidth,Freq,") + "Ops,KernelCycles," + "TimeKernelMs,TimeApiMs," +
                         "EffKernelPct,EffApiPct," + "PerfKernelTops,PerfApiTops\n";
        for (unsigned int i = 0; i < l_instrCount; ++i) {
            cout << "DATA_CSV:," << BLAS_ddrWidth << "," << l_boardFreqMHz << "," << l_maxCycleCount[i] << ","
                      << l_maxTimeKernelInMs[i] << "," << l_timeApiInMs << endl;
        }

    } else {
        cout << string("DATA_CSV:,DdrWidth,Freq,M,K,N,") + "Ops,KernelCycles," + "TimeKernelMs,TimeApiMs," +
                         "EffKernelPct,EffApiPct," + "PerfKernelTops,PerfApiTops\n";
        for (unsigned int i = 0; i < l_instrCount; ++i) {
    ...
    if (!getenv("SKIPPED_GOLD_CAL")) {
        ...
        cout << "INFO: skipped gold calculation on host since it may take too long\n" << endl;
    }
    return EXIT_SUCCESS;
}
