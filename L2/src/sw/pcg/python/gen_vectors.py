...
def process_matrices(isGenVec, isClean, mtxList, parEntries, vecPath):
...
def main(args):
    if (args.usage):
        print('Usage example:')
        print('python gen_vectors.py --gen_vec [--clean] --mtx_list ./test_matrices.txt --vec_path ./vec_dat')
    else:
        process_matrices(args.gen_vec, args.clean, args.mtx_list, args.par_entries, args.vec_path)
